        ...
        public static int SearchInsert(int[] nums, int target)
        {
            try
            {
                ...
                {                           //for binary search
                    ...
                    if (nums[mid] > target)
                    {            //if target is samller than mid element
                        j = mid - 1;
                        ans = mid;
                    }
                    else
        ...
        */
        ...
        public static string MostCommonWord(string paragraph, string[] banned)
        {
            try
            {
                ...
                Dictionary<string, int> map = new Dictionary<string, int>(); // Initializing  dictionary for mapping
                string temp = "";
                for (int i = 0; i < paragraph.Length; i++)
                {                     //iterating over the sentance character by charater
                    if (paragraph[i] == ',') continue;                         // condition for elemenating character so that punctuation is ignored
                    if (paragraph[i] == ' ' || paragraph[i] == '.')
                    {            //if this condition meets we got a word in temp variable
                        string new_string = change_to_small(temp);         //function call for changing each word in lowercase
                        ...
                        {
                            map[new_string] = 1;                           //else inser a new map value
                        }
                        temp = "";
                    }
                    else
                ...
                {                                   //iterating for finding the most frequent character
                    if (map[key] > max_fre)
                    {
                        max_fre = map[key];
                        ans = key;
        ...
        public static int FindLucky(int[] arr)
        {
            try
            {
                ...
                {                              //iterating over arr for mapping the value and its frequency
                ...
                int ans = -1;
                foreach (var key in val)
                {                                  //checking the condition for lucky number
        ...
        */
        ...
        public static string GetHint(string secret, string guess)
        {
            try
            {
                ...
                {                   //iterating over secret string
                    if (secret[i] == guess[i])
                    {                            // if character matches the increment value A
                        x++;
                        same_value[i] = true;
                    ...
                    else
                    {
                        ...
                        {                    //creating map for that
                            map[secret[i]] = map[secret[i]] + 1;
                        }
                        else
                        {
                            map[secret[i]] = 1;                                 //if map not found the creating one
                        }
                    }
                }
                // var val = map.Keys.ToList();
                ...
                string ans = x + "A" + y + "B";                           //concating string for Answer matching
                return ans;
            }
            catch (Exception)
            {
